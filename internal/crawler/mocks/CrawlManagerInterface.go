...
// CrawlURL provides a mock function with given fields: url, options
func (_m *CrawlManagerInterface) CrawlURL(url string, options *crawler.CrawlOptions) error {
	ret := _m.Called(url, options)
	...
	var r0 error
	if rf, ok := ret.Get(0).(func(string, *crawler.CrawlOptions) error); ok {
		r0 = rf(url, options)
	} else {
		r0 = ret.Error(0)
	}
